...
export const getDashboard = async (req, res) => {
  try {
    ...
    return res.json({dashboardData : {totalListings, totalRevenue, activeListings, totalUsers, recentListings}})
  } catch (error) {
    console.log(error);
    res.status(400).json({message: error.code || error.message});
  }
...
export const allUnchangedListings = async (req, res) => {
  try {
    const listings = await prisma.listing.findMany({
      where: {
...
// Change credentials for verified listings
...
export const getAllTransactions = async (req, res) => {
  try {
    ...
    //  Get customer detilas for each transaction and add it to the transaction object
...
// controller for marking withdrawl as paid
export const markWithdrawalAsPaid = async (req, res) => {
  try {
    ...
    if(!withdrawal){
      return res.status(400).json({message: "Withdrawl not found"});
    }

    if(withdrawal.isWithdrawn){
      return res.status(400).json({message: "Withdrawl already marked as paid"});
    }
