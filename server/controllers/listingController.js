...
export const addCredengtial = async (req, res) => {
  try {
    const {userId} = await req.auth();
...
export const getAllUserOrders = async (req, res) => {
  try {
    ...
    // Attache the credential to each order
    const credentials = await prisma.credential.findMany({
      where: {listingId: {in: orders.map((order)=>order.listingId)}}
    })

    const ordersWithCredentials = orders.map((order)=>{
      const credential = credentials.find((credential)=> credential.listingId === order.listingId)
...
export const purchaseAccount = async (req, res)=> {
   try {
     ...
     if(!listing){
      return res.status(400).json({message: "Listing not found or not active"});
     }
    ...
    const session = await stripeInstance.checkout.sessions.create({
      ...
      line_items: [        
        ...
        expires_at: Math.floor(Date.now() / 1000) + 30 * 60, // expires in 30 miniutes
      });
