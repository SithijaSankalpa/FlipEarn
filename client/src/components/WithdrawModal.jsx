...
const WithdrawModal = ({onClose}) => {
  ...
  const [account, setAccount] = useState([
    ...
    {type: "text", name: "SWIFT/IFSC Code", value: ""},
    {type: "text", name: "Branch", value: ""}
  ])

  const handleSubmisson = async (e) => {
    e.preventDefault();
    try {
  ...
  return (
    <div className='fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-100 flex items-center justify-center sm:p-4'>
        <div className="bg-white sm:rounded-lg shadow-2xl w-full max-w-2xl sm:max-h-[90vh] flex flex-col">
          {/* Header*/}
          ...
          {/* Form*/}
          <form onSubmit={handleSubmisson} className='flex flex-col items-start gap-4 p-4 overflow-y-auto'>
